...
from ovstest.util import start_process, interface_assign_ip, interface_up, move_routes, interface_get_ip
...
def ovs_vsctl_show():
    ...
    """
    ret, _out, _err = start_process(["ovs-vsctl", "show"])
    return _out


def ovs_vsctl_list_br():
    """
    This function shows names of all the bridges.
    """
    ret, _out, _err = start_process(["ovs-vsctl", "list-br"])
    return _out


def ovs_vsctl_list_ifaces(bridge):
    """
    This function shows names of all the interfaces.
    """
    ret, _out, _err = start_process(["ovs-vsctl", "list-ifaces", bridge])
    return _out


def ovs_vsctl_list_ports(bridge):
    """
    This function shows names of all the ports of the bridge.
    """
    ret, _out, _err = start_process(["ovs-vsctl", "list-ports", bridge])
    return _out
...
def ovs_vsctl_add_bridge(bridge):
    ...
    """
    ret, _out, _err = start_process(["ovs-vsctl", "add-br", bridge])
    return ret
...
def ovs_vsctl_del_bridge(bridge):
    ...
    """
    ret, _out, _err = start_process(["ovs-vsctl", "del-br", bridge])
    return ret
...
def ovs_vsctl_del_pbridge(bridge, iface):
    ...
    """
    (ip_addr, mask) = (bridge)
    interface_assign_ip(iface, ip_addr, mask)
    interface_up(iface)
    move_routes(bridge, iface)
...
def ovs_vsctl_is_ovs_bridge(bridge):
    ...
    """
    ret, _out, _err = start_process(["ovs-vsctl", "br-exists", bridge])
    return ret == 0
...
def ovs_vsctl_add_port_to_bridge(bridge, iface):
    ...
    """
    ret, _out, _err = start_process(["ovs-vsctl", "add-port", bridge,
                                          iface])
    return ret
...
def ovs_vsctl_del_port_from_bridge(port):
    ...
    """
    ret, _out, _err = start_process(["ovs-vsctl", "del-port", port])
    return ret
...
def ovs_vsctl_set(table, record, column, key, value):
    ...
    ret, _out, _err = start_process(["ovs-vsctl", "set", table, record,
                                          index_value])
    return ret
...
def ovs_get_physical_interface(bridge):
    ...
    """
    ret, out, _err = start_process(["ovs-vsctl", "list-ifaces", bridge])
    if ret == 0:
        ifaces = out.splitlines()
        for iface in ifaces:
            ret, out, _err = start_process(["ovs-vsctl", "get",
                                                 "Interface", iface, "type"])
            if ret == 0:
                #if ('""' in out) or ('system' in out):
                return iface.decode("utf-8")  # this should be the physical interface
    return None
