...
from .exception import OFCtlCmdExecError, OFCtlCmdParseError, VSCtlCmdExecError, VSCtlCmdParseError
from ovs_vsctl import utils


INSTALLED_OVS_OFCTL = find_executable("ovs-ofctl")
DEFAULT_OVS_OFCTL = '%s/bin/ovs-ofctl' % path.dirname(__file__)
...
class Ctl(object):
    ...
    def __init__(self, protocol='tcp', ip_addr='127.0.0.1', port=6640):
        ...
        self.ovs_ofctl_path = INSTALLED_OVS_OFCTL or DEFAULT_OVS_OFCTL
    ...
    def ofctl_run(self, command, table_format='list', data_format='string',
            parser=None):
        """
        """
        # Constructs command.
        ...
        process = utils.run(args)
        if process.returncode != 0:
            raise OFCtlCmdExecError(process.stderr.read())
...
if __name__ == '__main__':
    name = find_executable("ovs-ofctl")
    args = [name, 'show', 'br1']
    process = utils.run(args)
    if process.returncode != 0:
        raise OFCtlCmdExecError(process.stderr.read())
