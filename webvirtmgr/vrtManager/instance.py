    ...
    def get_net_device(self):
        def get_mac_ipaddr(net, mac_host):
            def fixed(ctx):
                for net in ctx.xpath('/network/ip/dhcp/host'):
                    mac = net.xpath('@mac')[0].content
                    host = net.xpath('@ip')[0].content
                    if mac == mac_host:
                        return host
                return None
        ...
        def networks(ctx):
            ...
            for net in ctx.xpath('/domain/devices/interface'):
                mac_host = str(net.xpath('mac/@address')[0])
                nic_host = str(net.xpath('source/@network|source/@bridge|source/@dev')[0])
                try:
    ...
    def get_disk_device(self):
        def disks(ctx):
            ...
            for disk in ctx.xpath('/domain/devices/disk'):
                device = str(disk.xpath('@device')[0])
                if device == 'disk':
                    try:
                        dev = disk.xpath('target/@dev')[0].content
                        src_fl = disk.xpath('source/@file|source/@dev|source/@name|source/@volume')[0].content
                        disk_format = disk.xpath('driver/@type')[0].content
                        try:
    ...
    def get_media_device(self):
        def disks(ctx):
            ...
            for media in ctx.xpath('/domain/devices/disk'):
                device = str(media.xpath('@device')[0])
                if device == 'cdrom':
                    try:
