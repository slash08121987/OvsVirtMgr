...
class wvmConnectionManager(object):
    ...
    def _search_connection(self, host, login, passwd, conn):
        ...
        self._connections_lock.reader_lock.acquire()
        try:
            ...
            # self._connections_lock.writer_lock.release()
        return None

    def get_connection(self, host, login, passwd, conn):
        """
        returns a connection object (as returned by the libvirt.open* methods)
        for the given host and credentials
        ...
        if (connection is None):
            self._connections_lock.writer_lock.acquire() # acquireWrite()
            try:
